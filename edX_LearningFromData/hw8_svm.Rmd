...
Lets Run this one manually
```{r q02, dependson="loadData", cache=T, fig.width=12}
library(e1071)
library(caret)
...
Code belw runs the model with various costs and degree=2.

```{r varycost, dependson="dat15", fig.width=12}
require(foreach)
Costs=10^(-4:0)
...
mysvm = getSvm ( trnData = trn15, Cost = 10^-3  )
...
```{r q06,dependson="varycost", dependson="fncSV", fig.width=12}
Costs=10^(-4:0)
f5 = function(x) {
...
s <-getSvm( trn15, Cost=1, degree=5)

# for some reason this just duplicates previous plot
# update: this works if we define X,y in parent environment or closure
p<-svm_plot(s, trn15)
print(p)
...
Used this code to decide submission.
```{r manTune, dependson="dat15", cache=T, fig.width=12}
library(foreach)
library(data.table)
REPS=100
Costs=10^(-4:0)
...
The code below uses svm cross validataion parameter to get winner in each run.
Runs are manual.

```{r useSVMCross, dependson="dat15", cache=T, fig.width=12}
library(foreach)
library(data.table)
REPS=100
Costs=10^(-4:0)
...
```{r RBF, dependson="dat15", fig.width=12}
require(e1071)
require(foreach)
require(ggplot2)
require(gridExtra)
...
The Plot below is for cost = 10e6
```{r pltCost_6, dependson="RBF", fig.width=12}
s_p6 = getRBF(trn15, Cost=10^6)
p1=svm_plot(s_p6, trn15)
print(p1)

```


The plot Below is for cost=100
```{r pltCost_2, dependson="RBF", fig.width=12}
s_p2 <- getRBF(trn15, Cost=10^2)
p2<-svm_plot(s_p2, trn15)
print(p2)
```

Code below still does not work even with environemtn trick. will need to define a real 
closure for this to work.
```{r combinedPlot, eval=FALSE}
require(gridExtra)
grid.arrange( p1, p2, ncol=2, main = textGrob("SVM DB with 10^6 and 10^2"))
