...
```{r preProcess, cache=TRUE, dependson="loadData"}
require(caret)
...
```{r visPCA, dependson="preProcess", fig.height=5, fig.width=7, cache=TRUE}
...
```{r svmMod, dependson="preProcess", cache=T}
...
```{r tunePolt, cache=TRUE, dependson="svmMod", echo=FALSE}
require(caret)
plot(csvm5)
```
...
```{r accuracyMeasure, dependson="svmMod", warning=FALSE, message=FALSE}
require(caret)
